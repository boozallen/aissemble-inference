...
A working example demonstrating object detection using the aiSSEMBLE Inference (OIP) library with a real MLServer deployment and YOLOv8 model.
...
2. Use the `aissemble-inference-yolo` module for YOLO model support
...
> **Note:** For an overview of tensor abstraction in aiSSEMBLE Inference, see the [main project README](../../README.md#-key-value-proposition-tensor-abstraction). For detailed technical examples showing two different tensor formats with identical user code, see [TENSOR_ABSTRACTION.md](./TENSOR_ABSTRACTION.md).
...
Note: The YOLORuntime is provided by the `aissemble-inference-yolo` module, not embedded in this example.
...
This will install:
- `aissemble-inference-core` (from local source)
- `aissemble-inference-yolo` (YOLO module with runtime)
- `mlserver` (inference server)
- `ultralytics` (YOLOv8, via aissemble-inference-yolo)
...
### YOLORuntime (from aissemble-inference-yolo)

The `aissemble-inference-yolo` module provides the `YOLORuntime` class that:

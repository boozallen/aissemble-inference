...
def given_kserve_and_custom_dataplane_handler(context):
    ...
    context.kserve_handler = KServeDataplaneHandler(handler=context.handler)


@when("model_ready is executed using the KServeDataplaneHandler")
def when_model_ready_using_kserve_handler(context):
    loop = asyncio.get_event_loop()
    context.result = loop.run_until_complete(
        context.kserve_handler.model_ready(model_name="test_model")
    )


@when("model_metadata is executed using the KServeDataplaneHandler")
def when_model_metadata_using_kserve_handler(context):
    loop = asyncio.get_event_loop()
    context.result = loop.run_until_complete(
        context.kserve_handler.model_metadata(model_name="test_model")
    )


@when("infer is executed using the KServeDataplaneHandler")
def when_infer_using_kserve_handler(context):
    ...
    loop = asyncio.get_event_loop()
    context.result = loop.run_until_complete(
        context.kserve_handler.infer(model_name="test_model", request=infer_request)
    )


@when("server_ready is executed using the KServeDataplaneHandler")
def when_server_ready_using_kserve_handler(context):
    loop = asyncio.get_event_loop()
    context.result = loop.run_until_complete(context.kserve_handler.ready())


@when("server_live is executed using the KServeDataplaneHandler")
def when_server_live_using_kserve_handler(context):
    loop = asyncio.get_event_loop()
    context.result = loop.run_until_complete(context.kserve_handler.live())


@when("server_metadata is executed using the KServeDataplaneHandler")
def when_server_metadata_using_kserve_handler(context):
    context.result = context.kserve_handler.metadata()
